...
function source(input = {}, env = process.env) {
  const configured = env.UNIPET_CODEX_SOURCE;
  if (configured) return cleanToken(configured, DEFAULT_SOURCE);
  if (envBool(env.UNIPET_CODEX_PER_SESSION, false) && input.session_id) {
    return cleanToken(`codex-${input.session_id}`, DEFAULT_SOURCE);
  }
...
function postJson(payload, env = process.env) {
  ...
  const timeoutMs = envInt(env.UNIPET_CODEX_TIMEOUT_MS, DEFAULT_TIMEOUT_MS, 50, 10000);
...
function startUnipet(env = process.env) {
  const command = env.UNIPET_COMMAND || env.UNIPET_CODEX_UNIPET_COMMAND || 'unipet';
...
async function sendHookEvent(eventName, input = {}, env = process.env) {
  ...
  try {
    ...
    if (payload.action === 'remove' || !envBool(env.UNIPET_CODEX_AUTO_START, true)) {
      return 0;
    }
  }
...
async function main(argv = process.argv.slice(2), env = process.env) {
  ...
  if (!eventName || eventName === 'help' || eventName === '--help' || eventName === '-h') {
    console.log('Usage: unipet hook codex <session_start|user_prompt_submit|pre_tool_use|permission_request|post_tool_use|stop>');
